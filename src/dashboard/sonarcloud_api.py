...
class SonarCloudAPI:
    ...
    async def _get_organization_projects_async(self, organization: str) -> List[SonarProject]:
        projects = []
        page_size = 500
        ...
        async with aiohttp.ClientSession(headers=self.headers) as session:
            try:
                first_page = await self._fetch_page(session, url, params)
                ...
                if total > page_size:
                    ...
                    for page in range(2, total_pages + 1):
                        p = params.copy()
                        p['p'] = page
                        tasks.append(self._fetch_page(session, url, p))
    ...
    def get_organization_projects(self, organization: str) -> List[SonarProject]:
        try:
    ...
    async def _get_project_measures_async(self, project_key: str, branch: str = None) -> Optional[Dict[str, float]]:
        try:
            params = {
    ...
    def get_project_measures(self, project_key: str, branch: str = None) -> Optional[Dict[str, float]]:
        try:
            loop = asyncio.get_running_loop()
            return loop.run_until_complete(self._get_project_measures_async(project_key, branch))
        except RuntimeError:
            return asyncio.run(self._get_project_measures_async(project_key, branch))
    
    async def _get_project_history_async(self, project_key: str, days: int, branch: str = None) -> List[Dict]:
        ...
        try:
            ...
            async with aiohttp.ClientSession(headers=self.headers) as session:
                while True:
                    ...
                    response = await self._fetch_page(session, url, params)
    ...
    def get_project_history(self, project_key: str, days: int, branch: str = None) -> List[Dict]:
        try:
            loop = asyncio.get_running_loop()
            return loop.run_until_complete(self._get_project_history_async(project_key, days, branch))
        except RuntimeError:
            return asyncio.run(self._get_project_history_async(project_key, days, branch))
    
    def get_organization_metrics(self, organization: str) -> OrganizationMetrics:
        try:
    ...
    async def _get_project_branches_async(self, project_key: str) -> List[SonarBranch]:
        try:
    ...
    def get_project_branches(self, project_key: str) -> List[SonarBranch]:
        try:
            loop = asyncio.get_running_loop()
            return loop.run_until_complete(self._get_project_branches_async(project_key))
        except RuntimeError:
            return asyncio.run(self._get_project_branches_async(project_key))
