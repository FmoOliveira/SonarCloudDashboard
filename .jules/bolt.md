...
- **Anti-Pattern Found:** Utilizing $O(N)$ boolean indexing (`plot_data[plot_data['project_name'] == project]`) inside a nested loop over metrics and projects in Plotly chart generation. This created an $O(M \times P \times N)$ execution time block during rendering.
- **The Fix:** Pre-grouped the Pandas dataframe into a dictionary before the loops using `dict(tuple(plot_data.groupby('project_name')))`. This changes the inner loop extraction to an $O(1)$ dictionary lookup.
- **Why it Matters:** Streamlit is highly susceptible to main-thread blocking. Removing repeated $O(N)$ dataframe scans during chart generation ensures the UI remains responsive even when displaying multi-project, multi-metric dashboard views.
