...
* When a user selects a single date in a Streamlit `st.date_input` configured for a range, Streamlit temporarily reruns and returns a tuple of length 1. To prevent partial state confusion and invalid form submissions, explicitly check `isinstance(date_vals, tuple) and len(date_vals) == 1`, render an instructional `st.info` message, and conditionally set `disabled=True` on dependent form submit buttons.
