...
*   **Streamlit Custom Date Range State**: When using `st.date_input` for a date range, selecting a single date causes Streamlit to temporarily return a tuple of length 1. This partial state can lead to unexpected behavior or invalid form submissions if not handled. Always explicitly check for this intermediate state (e.g., `elif isinstance(date_vals, tuple) and len(date_vals) == 1:`), provide an instructional `st.info` message, and conditionally disable dependent submit buttons until the user completes the selection.
